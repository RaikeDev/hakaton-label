...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#111722] border border-[#2A3242] rounded-md px-4 py-3 shadow-xl text-xs">
        <div className="text-[#8B93A3] mb-1">{label}</div>
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          const Icon = kpi.icon;
          return (
            <div key={i} className="bg-[#10141D] border border-[#202633] rounded-lg p-5">
              <div className="flex items-start justify-between mb-3">
                <div className="w-9 h-9 rounded-md bg-[#151B26] flex items-center justify-center">
              ...
              <div className="text-[#B5BCC9] text-sm mt-1">{kpi.label}</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-[#10141D] border border-[#202633] rounded-lg p-5">
          ...
          <p className="text-[#8B93A3] text-xs mb-4">Все платформы · руб.</p>
        ...
        <div className="bg-[#10141D] border border-[#202633] rounded-lg p-5">
          ...
          <p className="text-[#8B93A3] text-xs mb-4">Все платформы</p>
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={monthlyRevenue} margin={{ left: -15, right: 5 }}>
              <defs>
                <linearGradient id="streamGrad" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#4B8BFF" stopOpacity={0.25} />
                  <stop offset="95%" stopColor="#4B8BFF" stopOpacity={0} />
                </linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" stroke="#202633" />
              <XAxis dataKey="month" tick={{ fill: "#747D8C", fontSize: 11 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fill: "#747D8C", fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(v) => fmtStreams(v)} />
              <Tooltip content={<CustomTooltip />} />
              <Area type="monotone" dataKey="streams" stroke="#4B8BFF" fill="url(#streamGrad)" strokeWidth={2} dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-[#10141D] border border-[#202633] rounded-lg p-5">
          ...
          <p className="text-[#8B93A3] text-xs mb-4">По стране слушателя</p>
          <div className="space-y-3">
            {geoData.map((g) => (
              <div key={g.country}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[#B5BCC9] text-sm">{g.country}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-[#8B93A3] text-xs">{fmtStreams(g.streams)}</span>
      ...
      <div className="bg-[#10141D] border border-[#202633] rounded-lg p-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-[#747D8C] text-xs border-b border-[#202633]">
            ...
            <tbody>
              {(analyticsData?.tracks ?? []).slice(0, 5).map((track: any) => (
                <tr key={track.track_id} className="border-b border-[#202633]/80 hover:bg-[#151B26]/70 transition-colors">
                  <td className="py-3 text-white font-medium">{track.title}</td>
                  <td className="py-3 text-right text-[#B5BCC9]">{fmtStreams(track.streams)}</td>
