    ...
    <div className="flex-1 overflow-hidden flex flex-col">
      ...
      <div className="px-6 pt-6 pb-4 border-b border-[#1C1A3B] shrink-0">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[#6C6890] text-sm mt-0.5">{isLoading ? "Загрузка..." : `${(tracks as any[]).length} треков · все платформы`}</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setShowUpload(true)}
              className="flex items-center gap-2 px-4 py-2 bg-[#0F0D22] border border-[#1C1A3B] text-[#9B98BC] text-sm rounded-xl hover:border-[#252356] hover:text-white transition-all"
            >
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1 max-w-xs">
            <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6C6890]" />
            <input
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Найти трек..."
              className="w-full bg-[#0F0D22] border border-[#1C1A3B] text-white placeholder-[#4A4469] text-sm rounded-xl pl-9 pr-4 py-2.5 focus:outline-none focus:border-violet-500/50 transition-colors"
            />
          </div>
          <button className="flex items-center gap-2 px-3 py-2.5 bg-[#0F0D22] border border-[#1C1A3B] text-[#9B98BC] text-sm rounded-xl hover:border-[#252356] transition-all">
            <Filter size={14} />
            Фильтр
          </button>
          <button className="flex items-center gap-2 px-3 py-2.5 bg-[#0F0D22] border border-[#1C1A3B] text-[#9B98BC] text-sm rounded-xl hover:border-[#252356] transition-all">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          {/* Column headers */}
          <div className="flex items-center gap-3 px-6 py-3 border-b border-[#1C1A3B] text-[#4A4469] text-xs font-medium sticky top-0 bg-[#06050F] z-10">
                ...
                <div
                  ...
                  className={`flex items-center gap-3 py-3 px-3 rounded-xl cursor-pointer -mx-3 transition-all border ${
                    isSelected
                      ...
                      : "border-transparent hover:bg-[#0F0D22]"
                  }`}
                >
                  <span className="text-[#4A4469] text-sm font-mono w-6 shrink-0 text-center">{idx + 1}</span>
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <Clock size={11} className="text-[#4A4469]" />
                        <span className="text-[#6C6890] text-xs">{track.duration}</span>
                        <span className="text-[#4A4469] text-xs">· ISRC: {track.isrc}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="w-20 text-right text-[#6C6890] text-xs">
                  ...
                  <button className="w-8 flex justify-center text-[#4A4469] hover:text-white transition-colors">
        ...
        {/* Track detail panel */}
        {selectedTrackData && (
          <div className="w-72 shrink-0 border-l border-[#1C1A3B] bg-[#09071C] overflow-y-auto p-5">
            <div className="flex items-start justify-between mb-4">
              <h3 className="text-white font-semibold">Детали трека</h3>
              <button onClick={() => setSelectedTrack(null)} className="text-[#4A4469] hover:text-white transition-colors">
                <X size={16} />
              </button>
            </div>
            <img
            ...
            <p className="text-[#6C6890] text-sm mb-4">MAKO · {selectedTrackData.duration}</p>

            <div className="space-y-3">
              <div className="bg-[#0F0D22] rounded-xl p-3 border border-[#1C1A3B]">
                <div className="text-[#6C6890] text-xs mb-2">По платформам</div>
                {Object.entries(selectedTrackData.platform_streams ?? {}).map(([pid, streams]) => {
                  const sNum = streams as number;
                  const total = (Object.values(selectedTrackData.platform_streams ?? {}) as number[]).reduce((a, b) => a + b, 0);
                  const pct = total > 0 ? ((sNum / total) * 100).toFixed(0) : "0";
                  return (
                    <div key={pid} className="mb-2">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[#9B98BC] text-xs">{pid}</span>
                      ...
                      <div className="h-1.5 bg-[#1C1A3B] rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${pct}%`, background: PLATFORM_COLORS[pid] || "#6C6890" }}
                        />
                      </div>
                    </div>
                  );
          ...
          <div className="bg-[#09071C] border border-[#1C1A3B] rounded-2xl w-full max-w-lg">
            <div className="flex items-center justify-between p-5 border-b border-[#1C1A3B]">
              <div>
                ...
                <p className="text-[#6C6890] text-sm mt-0.5">Drag & drop или выберите файлы</p>
              </div>
              <button onClick={() => { setShowUpload(false); setUploadedFiles([]); }} className="text-[#4A4469] hover:text-white transition-colors">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <div
                ...
                className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${
                  dragOver ? "border-violet-500 bg-violet-500/10" : "border-[#252356] hover:border-violet-500/50 hover:bg-violet-500/5"
                }`}
              >
                <Upload size={32} className="text-[#4A4469] mx-auto mb-3" />
                <p className="text-white font-medium mb-1">Перетащите файлы сюда</p>
                <p className="text-[#6C6890] text-sm">MP3, WAV, FLAC или CSV/XLSX для батч-загрузки</p>
                <input
                  ref={fileRef}
                  type="file"
                  multiple
              ...
              <div className="bg-[#0F0D22] border border-[#1C1A3B] rounded-xl p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[#6C6890] text-xs">Загрузите файл с треками: название, ISRC, дата релиза, платформы — всё автоматически распарсится.</p>
              ...
              {uploadedFiles.length > 0 && (
                <div className="space-y-2">
                  {uploadedFiles.map((f, i) => (
                    <div key={i} className="bg-[#0F0D22] border border-[#1C1A3B] rounded-xl p-3">
                      <div className="flex items-center gap-3">
                        <Music size={16} className="text-violet-400 shrink-0" />
                        <div className="flex-1 min-w-0">
                          <div className="text-white text-sm truncate">{f.name}</div>
                          <div className="text-[#6C6890] text-xs">{f.size}</div>
                        </div>
                        {f.status === "done" ? (
                          <CheckCircle size={16} className="text-emerald-400 shrink-0" />
                        ) : (
                          <span className="text-[#6C6890] text-xs shrink-0">{Math.round(f.progress)}%</span>
                        )}
                      </div>
                      {f.status === "uploading" && (
                        <div className="h-1 bg-[#1C1A3B] rounded-full overflow-hidden mt-2">
                          <div
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  className="flex-1 py-2.5 border border-[#1C1A3B] text-[#9B98BC] rounded-xl hover:bg-[#0F0D22] transition-colors text-sm"
                >
                  Отмена
                </button>
                <button className="flex-1 py-2.5 bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 text-white rounded-xl transition-colors text-sm font-medium">
                  Продолжить
                </button>
              </div>
            </div>
